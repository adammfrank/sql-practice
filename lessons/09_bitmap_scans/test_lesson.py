...
def test_solution(lesson_db):
    ...
    grader.assert_plan(p, must_have=["Bitmap Heap Scan", "BitmapAnd"])
    measured = timing.measure_execution_ms(conn, lesson.solution_sql)
    # Brief's suggested ratio=5 is not achievable on this dataset: `orders`
    # is small enough (~34MB) that a full seq scan is already cheap, and a
    # ~1/36-selective BitmapAnd measures ~1.6-2x faster in repeated runs on
    # the seeded template (see README step 5). ratio=1.5 is set below that
    # measured floor so the gate is real but not flaky.
